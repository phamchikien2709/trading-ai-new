...
def test_add_robustness_median_of_neighbours():
    ...
    out = add_robustness(_robust_df(), grid)
...
def test_add_robustness_excludes_ruined_neighbours():
    grid = GridSpec(tp_r=(1.0, 2.0), atr_mult=(0.5, 1.0), rsi_slow_levels=((75, 25),), rsi_fast_levels=((90, 10),))
    out = add_robustness(_robust_df(ruined=[False, True, False, False]), grid)
...
def test_add_robustness_groups_by_all_four_rsi_levels():
    ...
    grid = GridSpec(tp_r=(1.0, 2.0), atr_mult=(0.5, 1.0),
                    rsi_slow_levels=((75, 25), (75, 20)), rsi_fast_levels=((90, 10), (90, 5)))
...
def test_recommend_rejects_money_losing_and_ruined_combos(bad):
    ...
    assert recommend(_grid_df([bad])) == {"M5": None}
    assert recommend(_grid_df([{}]))["M5"] is not None       # the same row without the defect passes
...
def test_reason_marks_a_grid_edge_pick():
    rec = recommend(_grid_df([{"grid_edge": True}]))["M5"]
